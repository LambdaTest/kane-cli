...
> Requires kane-cli **0.6.1 or later** (`kane-cli --version`). On 0.6.0 these commands fail after a fresh install — upgrade. This guide describes **0.8.6**; version-specific behavior is marked with the release that introduced it — *(0.7.1)* through *(0.8.6)* — on its page.
