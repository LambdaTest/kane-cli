...
- **Linear issues, by URL** *(0.8.6)*: `kane-cli context ingest https://linear.app/<workspace>/issue/KEY-123` — slug, query, and `#comment` variants of the same issue all converge on one source. A Linear connection for your account is required; without one the ingest refuses up front and points you at the Integrations screen. Ingested: the title, description (every line citable), workflow fields (status, priority, project, labels), **all comments including threaded replies** — comments are part of the issue's identity, so a failed or partial comments read refuses the whole ingest — and images, inline and attached, as citable parts; attachment links to other apps are listed, never fetched. Identity follows the issue itself: renaming a team key continues the *same* source (`versioned`), while a recycled key — the same key pointing at a different issue — refuses with a retire/`--as` recovery. The default source id is the lowercased key (`eng-42`); `--as` overrides.
- **Linear documents, by URL** *(0.8.6)*: `kane-cli context ingest https://linear.app/<workspace>/document/<slug>` — the slug must end in the document's 12-hex id. The connection needs document access (the refusal carries a resync hint otherwise). Ingested: the document body (every line citable) plus its inline images. The default id is `doc-<id>`; `--as` overrides. Workspace pages — project, team, or view URLs — aren't ingestable and refuse loudly: ingest their issues or documents individually.
- **Web pages, by URL** *(0.8.3)*: any public page URL that isn't a Jira issue, Confluence page, or Linear issue/document — `kane-cli context ingest https://docs.example.com/guide`. The page is fetched and converted to text by the service (the CLI never fetches pages itself), with every line citable. The default id is a slug of the URL plus a short hash, so the same URL always converges on the same source; `--as` adopts a custom id, and re-ingesting a **different** URL under an adopted id asks first on a terminal (headless it refuses — keep the pages under separate ids instead). Re-ingesting an unchanged page is `unchanged`; a content change is `versioned` with staleness flowing to items extracted from the old snapshot — title, redirect, status, or timestamp changes never move the head. Always the live page: there is no point-in-time read. Inaccessible pages — not found, behind a login or paywall, bot-blocked, non-HTML, too large, timing out — refuse with the reason in plain language, and private or internal addresses are refused.
- **Remote ids share one space** *(0.8.6)*: a Jira `ENG-42` and a Linear `ENG-42` both mint `eng-42`, so a URL whose id is already backed by a different kind of source refuses with a recovery — retire the existing source, or adopt the new one under `--as` — never a silent replace. (linear.app pages ingested as *web* sources before 0.8.6 remain separate sources: retire them or keep both.)
...
The chat has two zones. The **scrollback** is the complete session journey — source banners, the agent's narrative, its reasoning and tool lines, each turn's proposals as an **item table**, answer receipts, commit receipts, and the closing session summary. The **live region** below holds only what's happening now: the working line (**ctrl+t** expands the agent's reasoning), and one panel.

*(0.8.2)* A compact three-line status bar sits between the scrollback and the live region, aggregating the session's state; its last line lists **only the keys that work right now**, so the legend is always true.
...
**Ctrl+C asks for a pause, not a crash.** The first press asks the agent to save the session; a second Ctrl+C at any point is an immediate hard exit. Finishing cleanly is the idle panel's row 1. *(0.8.2)* However a session ends — finished, paused, interrupted, or crashed — it prints one **session summary** block into the scrollback: the state, what happened (paused, crashed, and held summaries include the credits used), the exact resume command whenever the session is resumable (including the `--message` form), and the trace line. A pause exits `3`; a save that can't complete within a few seconds exits `130` with an honest `interrupted — session not saved`.
