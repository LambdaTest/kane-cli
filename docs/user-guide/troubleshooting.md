...
- **A member reported `broken` with no steps, nothing published** — the grid-side kane-cli refused before launching. Open the printed job link and read the scenario stage's log. For mobile members the usual cause is an `APP…` id uploaded on a different environment or organisation than `--env`; `kane-cli apps list --target <kind>` for the active profile is the authority. The members' session logs are also under `~/.testmuai/kaneai/sessions/remote/<job-id>/`.
