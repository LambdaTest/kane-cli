...
- A **designed** test failed a run (its `_test.md` carries an `assurance:` block) → Read `references/assurance.md` §6.1 **before touching the file** — a hand edit is adopted silently as the new design; route the failure through redesign or reconcile, and edit by hand only after reading `references/objectives-cookbook.md`
