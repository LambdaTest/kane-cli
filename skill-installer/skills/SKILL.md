...
- Share the context store with a team, join a teammate's, keep two stores level, or resolve a sync conflict → Read `references/context-sync.md` first — `kane-cli context sync|push|pull|clone` (kane-cli 0.8.14+); the store is shared through a location, never by copying or git-merging `.context/`
