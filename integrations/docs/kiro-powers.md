...
| `references/testrun.md` — batch runs: selection (paths/`--match`/`--tags`), preflight reasons, `--remote` (HyperExecute jobs, `remote_*` events, remote preflight codes), mobile members, `testrun_*` event schema, suite rollup presentation, exit codes | All of `steering/kane-cli-testrun.md` |
