...
| `references/testmd.md`: testmd file format, replay & cascade, `@import`, commands, parse errors, gate-fires-before-launch note, the saved-test stream table (`test_md_step_start` through `test_md_done`) | All of `steering/kane-cli-testmd.md` (the stream table lives under Replay policy and completion → The saved-test stream) |
